...
class ViewController: UIViewController, AdapteredCollectionDelegate {
	...
	func onEventReceived(eventType event: AdapteredCollectionView.AdapteredEvent!, object: BaseCollectionObject!, index: Int!, additionalObject: NSObject?) {
		...
		case 7:
			...
			let controller: UIViewController? = storyboard?.instantiateViewControllerWithIdentifier("SmileController");
			if (controller != nil) {
				self.navigationController?.presentViewController(controller!, animated: true, completion: nil);
			}
			break;
		case 8:
			self.performSegueWithIdentifier("toAdapteredTable", sender: self);
			break;
		default:
			break;
		}
